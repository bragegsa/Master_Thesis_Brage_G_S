...
%     lr=0.01;epochs=200;                   % learning rate and Itermax
    ...
    for epoch=1:epochs
        ...
        dropout = false;
        if dropout == true
            if AUC < min(AUC_values) && epoch > 30
...
% Training the network by BP
    ...
    delta1=grad.*x_hat.*(1-x_hat); % x_hat.*(1-x_hat) is derviate of sigmoid
    delta2=z*(L+L');
    tmp=(W2'*delta1+lamda*delta2).*(z.*(1-z)); % (z.*(1-z)) is derivate of sigmoid
