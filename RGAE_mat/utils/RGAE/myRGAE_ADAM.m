function y=myRGAE_ADAM(X,SG,lamda,hid, map, epochs, lr, idex, dataset_name, network_setup)%
    ...
    for epoch=1:epochs
        ...
        for j=1:batch_num
            ...
            [W1,b1,W2,b2,mt_prev_W1,mt_prev_b1,mt_prev_W2,mt_prev_b2,vt_prev_W1,vt_prev_b1,vt_prev_W2,vt_prev_b2] ...
                =trainAE(x,z,x_hat,W1,b1,W2,b2,res,s_G,lr,lamda,mt_prev_W1,mt_prev_b1,mt_prev_W2,mt_prev_b2,vt_prev_W1,vt_prev_b1,vt_prev_W2,vt_prev_b2, t);
...
%             
        ...
        if es == true
            if epoch > 2
    %             if epoch_loss < 0.2 && last_n_std < 0.003 % && epoch > 200
                if epoch_loss < 0.2 && last_n_std < 0.006 || epoch_loss > epoch_loss_curve(end-1) && epoch_loss < 0.5 && epoch > 50
...
%                 AUC_values(end + 1) = AUC;
    ...
    saveas(gcf, join([folder_path, '/', network_setup, '.png']));
%     close(gcf);
...
function [W1_,b1_,W2_,b2_,mt_W1,mt_b1,mt_W2,mt_b2,vt_W1,vt_b1,vt_W2,vt_b2]= ...
    trainAE(x,z,x_hat,W1,b1,W2,b2,res,L,lr,lamda,mt_prev_W1,mt_prev_b1,mt_prev_W2,mt_prev_b2,vt_prev_W1,vt_prev_b1,vt_prev_W2,vt_prev_b2, t)
...
%     [b2_, mt_b2, vt_b2] = ADAM(b2, delta1*ones(1,num)'/num, mt_prev_b2, vt_prev_b2, beta1, beta2, epsilon, lr);
    [W2_, mt_W2, vt_W2] = adamupdate(W2, delta1*z'/num, mt_prev_W2, vt_prev_W2, t, lr, beta1, beta2, epsilon);
    [b2_, mt_b2, vt_b2] = adamupdate(b2, delta1*ones(1,num)'/num, mt_prev_b2, vt_prev_b2, t, lr, beta1, beta2, epsilon);

%     [W1_, mt_W1, vt_W1] = ADAM(W1, tmp*x'/num, mt_prev_W1, vt_prev_W1, beta1, beta2, epsilon, lr);
%     [b1_, mt_b1, vt_b1] = ADAM(b1, tmp*ones(1,num)'/num, mt_prev_b1, beta1, beta2, epsilon, lr);
    [W1_, mt_W1, vt_W1] = adamupdate(W1, tmp*x'/num, mt_prev_W1, vt_prev_W1, t, lr, beta1, beta2, epsilon);
    [b1_, mt_b1, vt_b1] = adamupdate(b1, tmp*ones(1,num)'/num, mt_prev_b1, vt_prev_b1, t, lr, beta1, beta2, epsilon);
