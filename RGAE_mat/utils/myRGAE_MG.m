function y=myRGAE_MG(X,SG_1,SG_2,lamda,hid,map,epochs,lr,idex_1)%
...
%     lr=0.01;epochs=200;                   % learning rate and Itermax
    ...
    for epoch=1:epochs
        ...
        for j=1:batch_num
            ...
            s_G_1=SG_1(ind((j-1)*batch_size+1:j*batch_size),ind((j-1)*batch_size+1:j*batch_size));
            s_G_2=SG_2(ind((j-1)*batch_size+1:j*batch_size),ind((j-1)*batch_size+1:j*batch_size));
            % Forward
            z=sigmoid(W1,x,b1);
            ...
            [W1,b1,W2,b2,mt_prev_W1,mt_prev_b1,mt_prev_W2,mt_prev_b2,vt_prev_W1,vt_prev_b1,vt_prev_W2,vt_prev_b2] ...
                =trainAE(x,z,x_hat,W1,b1,W2,b2,res,s_G_1,s_G_2,lr,lamda,mt_prev_W1,mt_prev_b1,mt_prev_W2,mt_prev_b2,vt_prev_W1,vt_prev_b1,vt_prev_W2,vt_prev_b2);
        end
        ...
        zips=[idex_1,y_test'];            % Recover the original image with 'idex'
...
function [W1_,b1_,W2_,b2_,mt_W1,mt_b1,mt_W2,mt_b2,vt_W1,vt_b1,vt_W2,vt_b2]= ...
    trainAE(x,z,x_hat,W1,b1,W2,b2,res,L_1,L_2,lr,lamda,mt_prev_W1,mt_prev_b1,mt_prev_W2,mt_prev_b2,vt_prev_W1,vt_prev_b1,vt_prev_W2,vt_prev_b2)
% Training the network by BP
    ...
    delta2=z*(L_1+L_1');
    delta3=z*(L_2+L_2');
%     tmp=(W2'*delta1+lamda*delta3).*(z.*(1-z));
    tmp=(W2'*delta1+lamda*delta2+lamda*delta3).*(z.*(1-z));
